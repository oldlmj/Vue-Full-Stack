<template>
  <div class="product-detail">
    <div class="product-image">
      <!-- <img src="`../assets/products/${product.image_urlo}`" /> -->
    </div>
    <div class="product-information">
      <div>{{ product.product_name }}</div>
      <div>
        <b style="font-size: 45px" class="text-success">${{ product.cost }}</b>
      </div>
      <div>分類 {{ product.category }}</div>
...
export default {
  ...
  methods: {
    ...
    increaseQuantity() {
      if (this.currQuantity < this.product.inventory) {
        this.currQuantity++;
      }
    },
    addToCart() {
    ...
    changeInput() {
      const max = +this.product.inventory;
